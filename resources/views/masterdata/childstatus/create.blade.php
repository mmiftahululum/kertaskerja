...
<x-app-layout>
    <x-slot name="header">
        <h2 class="text-lg font-semibold text-gray-900">{{ __('Tambah Child Status') }}</h2>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div class="bg-white shadow-sm rounded-lg">
            <div class="p-6">
                ...
                <form action="{{ route('child-statuses.store') }}" method="POST" class="space-y-4">
                    @csrf
                     ...
                     <div>
                        ...
                        <select id="head_status_id" name="head_status_id" required
                            ...
                            @foreach($headStatuses as $h)
                                <option value="{{ $h->id }}"
                                    {{ (string) old('head_status_id', request('head_status_id')) === (string) $h->id ? 'selected' : '' }}>
                                    {{ $h->head_status_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div x-data="{ color: '{{ old('status_color', '#000000') }}' }">
